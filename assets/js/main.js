...
function hasFormEndpoint() {
  ...
  return action && action !== "PASTE_FORMSPREE_OR_GETFORM_ENDPOINT_HERE";
}

async function sendToFormService() {
...
function showSuccess(request, savedLocally, emailSent) {
  successPanel.dataset.savedLocally = String(savedLocally);
  successPanel.dataset.emailSent = String(emailSent);

  if (emailSent) {
    successMessage.textContent =
      "Your project request has been received. You selected " +
      request.selectedPackage +
      ". Eviternite will contact you by email to discuss the details and next steps.";
  } else {
    successMessage.textContent =
      "Your project request was saved locally for preview. Add a Formspree or Getform endpoint to send submissions to poqimedia@gmail.com.";
  }
...
form.addEventListener("submit", (event) => {
  ...
  window.setTimeout(async () => {
    ...
    try {
      ...
      showSuccess(request, savedLocally, emailSent);
    } catch (error) {
      formError.textContent =
        "The request was saved locally, but email delivery failed. Please check your Formspree/Getform endpoint.";
    }

    submitButton.disabled = false;
    submitButton.textContent = "Send Project Request";
    if (emailSent || !hasFormEndpoint()) {
      form.reset();
